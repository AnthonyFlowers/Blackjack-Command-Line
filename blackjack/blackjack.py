...
def getStartChips(chip_file_name):
    chipsFile = open(chip_file_name, "r")
    text = chipsFile.read()
    chipsFile.close()
    print(text)
    return text.split(':')[1]
...
def setChipsFile(name, newChipsAmount, chip_file_name):
    chipsFile = open(chip_file_name, "w")
...
def main():
    try:
        startChips = getStartChips('chips.txt') # Get start chips amount
        ...
        while True:
            ...
            game.dealCards()
        setChipsFile(game.playerList[0].getName(), game.playerChips[0], 'chips.txt')
